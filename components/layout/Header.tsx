...
const Header = () => {
  ...
  if (!mounted) {
    ...
    // Returning the static part of the header is a good compromise.
    return (
    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 py-4 px-6 md:px-12 glass-ui">
        <div className="container mx-auto flex justify-between items-center">
          ...
          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center space-x-8 text-foreground/80">
            {navLinks.map((link) => (
              <Link key={link.name} href={link.href} className="hover:text-foreground transition-colors duration-300">
                {link.name}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="md:hidden z-50">
              <button onClick={toggleMenu} className="relative w-[30px] h-[22px]" aria-label="Toggle menu">
                <motion.div
                  className="absolute left-0 w-full h-[3px] bg-foreground rounded-full"
                ...
                />
                <motion.div
                  className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-[3px] bg-foreground rounded-full"
        ...
        {isOpen && (
          ...
          >
            ...
            <motion.div
              ...
              exit="hidden"
            >
              <nav className="mt-24 flex flex-col items-center justify-center h-full">
                {navLinks.map((link) => (
                  <motion.div key={link.name} variants={mobileLinkVariants} className="w-full text-center py-4">
                    <Link
                      href={link.href}
                      className="text-3xl font-bold text-foreground/80 hover:text-royal-gold transition-colors duration-300"
                    >
                      {link.name}
                    </Link>
                  </motion.div>
